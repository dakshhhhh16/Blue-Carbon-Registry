...
const VerificationMap: React.FC<VerificationMapProps> = ({
  ...
  // Loading states for MultiStepLoader with corresponding images
  const loadingStates = [
    { text: "Initializing satellite imagery analysis...", image: ss1Image },
    { text: "Processing vegetation coverage data...", image: ss2Image },
    { text: "Analyzing soil composition and carbon density...", image: ss33Image },
    { text: "Evaluating ecosystem restoration progress...", image: ss4Image },
    { text: "Generating comprehensive analysis report...", image: ss5Image }
  ];
  ...
  const startAIAnalysisWorkflow = () => {
    ...
    // Step 1: Show analysis boxes immediately when starting
    setTimeout(() => {
      showAnalysisBoxes();
    }, 500);
    
    // Step 2: Zoom out to show wider area (1 second)
    setTimeout(() => {
      map.current!.easeTo({
        zoom: zoomOut,
        duration: 1000
    ...
    // Step 3: Pan around to simulate scanning with boxes visible (2-3 seconds)
    setTimeout(() => {
      // Update boxes position during right pan
      updateAnalysisBoxesPosition('right');
      map.current!.panBy([50, 0], { duration: 800 }); // pan right
    }, 1800);
    
    setTimeout(() => {
      // Update boxes position during left pan
      updateAnalysisBoxesPosition('left');
      map.current!.panBy([-100, 0], { duration: 1200 }); // pan left
    }, 2600);
    
    setTimeout(() => {
      // Update boxes position back to center
      updateAnalysisBoxesPosition('center');
      map.current!.panBy([50, 0], { duration: 800 }); // back to center
    }, 3800);

    // Step 4: Zoom back in with boxes still visible (1 second)
    setTimeout(() => {
      map.current!.easeTo({
    ...
    // Step 5: Open analysis popup after animation completes
    setTimeout(() => {
  ...
  // Update analysis boxes position during pan animations
  const updateAnalysisBoxesPosition = (direction: 'left' | 'right' | 'center') => {
    if (!map.current) return;
    ...
    // Adjust box positions based on pan direction for dynamic effect
    let offsetX = 0;
    switch (direction) {
      case 'right':
        offsetX = 0.005; // Shift boxes slightly right
        break;
      case 'left':
        offsetX = -0.005; // Shift boxes slightly left
        break;
      case 'center':
      default:
        offsetX = 0; // Center position
        break;
    }
      ...
      features: [
        {
          ...
          geometry: {
            type: "Polygon" as const,
            coordinates: [[
              [lng - 0.01 + offsetX, lat - 0.01], 
              [lng + 0.005 + offsetX, lat - 0.01], 
              [lng + 0.005 + offsetX, lat + 0.005], 
              [lng - 0.01 + offsetX, lat + 0.005], 
              [lng - 0.01 + offsetX, lat - 0.01]
            ]]
          },
        ...
        },
        {
          ...
          geometry: {
            ...
            coordinates: [[
              [lng + 0.005 + offsetX, lat - 0.005], 
              [lng + 0.02 + offsetX, lat - 0.005], 
              [lng + 0.02 + offsetX, lat + 0.01], 
              [lng + 0.005 + offsetX, lat + 0.01], 
              [lng + 0.005 + offsetX, lat - 0.005]
            ]]
          },
          properties: { color: "#22c55e", type: "concern", label: "Monitoring Required" }
        },
        {
          type: "Feature" as const,
          geometry: {
            ...
            coordinates: [[
              [lng - 0.005 + offsetX, lat + 0.005], 
              [lng + 0.01 + offsetX, lat + 0.005], 
              [lng + 0.01 + offsetX, lat + 0.02], 
              [lng - 0.005 + offsetX, lat + 0.02], 
              [lng - 0.005 + offsetX, lat + 0.005]
            ]]
          },
          properties: { color: "#22c55e", type: "restored", label: "Restoration Success" }
        },
        {
          type: "Feature" as const,
          geometry: {
            ...
            coordinates: [[
              [lng - 0.02 + offsetX, lat + 0.01], 
              [lng - 0.005 + offsetX, lat + 0.01], 
              [lng - 0.005 + offsetX, lat + 0.025], 
              [lng - 0.02 + offsetX, lat + 0.025], 
              [lng - 0.02 + offsetX, lat + 0.01]
            ]]
          },
          properties: { color: "#22c55e", type: "growth", label: "Active Growth" }
        },
        {
          type: "Feature" as const,
          geometry: {
            ...
            coordinates: [[
              [lng + 0.01 + offsetX, lat - 0.02], 
              [lng + 0.025 + offsetX, lat - 0.02], 
              [lng + 0.025 + offsetX, lat - 0.005], 
              [lng + 0.01 + offsetX, lat - 0.005], 
              [lng + 0.01 + offsetX, lat - 0.02]
            ]]
          },
          properties: { color: "#22c55e", type: "carbon", label: "High Carbon Density" }
        }
      ]
    };

    // Update the existing analysis boxes data
    if (map.current.getSource('analysis-boxes')) {
      (map.current.getSource('analysis-boxes') as maplibregl.GeoJSONSource).setData(analysisBoxesData);
    }
  };
  ...
  const showAnalysisBoxes = () => {
    ...
    // Add ONLY border layer (no fill layer) for boundaries only
    map.current.addLayer({
  ...
  const startImageAnalysisSequence = () => {
    // MultiStepLoader will run for 7.5 seconds (5 steps × 1.5 seconds each)
    // Then transition to insights step
    setTimeout(() => {
      setAnalysisStep('insights');
      setShowInsights(true);
    }, 7500); // 7.5 seconds total for MultiStepLoader
  };
            ...
            <div className="p-6 overflow-auto max-h-[calc(90vh-120px)]">
              ...
              {analysisStep === 'insights' && (
                <div className="space-y-8">
                  ...
                  {/* Final Analysis Image - Full Width */}
                  ...
                  {/* Generated Insights - Checklist Table Format */}
                  <div className="space-y-4">
                    <h5 className="font-semibold text-gray-800 flex items-center justify-center">
                      <Eye className="h-4 w-4 mr-2 text-green-600" />
                      Verification Checklist & Key Insights
                    </h5>
